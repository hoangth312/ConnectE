...
- (void) startDiscovery
{
    NSLog(@"startDiscovery called");

    if (_searching) {
        NSLog(@"_searching is YES, returning");
        return;
    }

    NSLog(@"_searching is NO, continuing");

    if (_deviceClasses.count == 0) {
        NSLog(@"_deviceClasses is empty, registering default services");
        [self registerDefaultServices];
    } else {
        NSLog(@"_deviceClasses already has %lu items", (unsigned long)_deviceClasses.count);
    }

    _searching = YES;
    NSLog(@"_searching set to YES");

    NSLog(@"Starting discovery for all discovery providers");
    [_discoveryProviders enumerateObjectsUsingBlock:^(DiscoveryProvider *service, NSUInteger idx, BOOL *stop) {
        NSLog(@"Starting discovery for service at index %lu", (unsigned long)idx);
        [service startDiscovery];
    }];

    NSLog(@"Starting app state change listener");
    [self.appStateChangeNotifier startListening];

    NSLog(@"startDiscovery completed");
}
